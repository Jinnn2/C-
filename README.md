...
旧V6含当日实际值泄漏和次日预测错位，不作为有效主结果。修正版见[实验报告](reports/q2_v6_corrected_experiment.md)、[结果工作簿](results/result2_v6_corrected.xlsx)及`results/q2_v6_corrected/`。已改为对应提前期残差、逐候选完整闭环选参、可完全关闭纠偏和凸终值插值；一月选择关闭纠偏。334天总费用14,575,749.45元，比V2B减少11,602.51元（约0.07954%）。保留跨日采购方向，尚无日内DP纠偏收益证据。复现命令仍为`python scripts/solve_q2_v6.py`；针对性验证为`python scripts/validate_q2_v6.py`。
